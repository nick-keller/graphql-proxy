...
describe('createProxy arguments checks', () => {
  ...
  test('getter with arguments', () => {
    expect(() => createProxy({ getters: { foo(bar) {
      return bar
    } }})).toThrow(
  ...
  test('arrow getter', () => {
    expect(() => createProxy({ getters: { foo: () => 'bar' }})).toThrow(
      ...
      'but getter "foo" is. ' +
      'Replace its declaration with "foo() {...}" or ' +
      '"foo: function() {...}" in order to enable "this" binding.'
    )
  })

  test('valid getters', () => {
    expect(() => createProxy({ getters: { foo: function() {
      return 'bar'
    } }})).not.toThrow()
    expect(() => createProxy({ getters: { foo() {
      return 'bar'
    } }})).not.toThrow()
  })
  ...
  test('valid methods', () => {
    expect(() => createProxy({ methods: { foo: function() {
      return 'bar'
    } }})).not.toThrow()
    expect(() => createProxy({ methods: { foo(bar) {
      return bar
    } }})).not.toThrow()
  })
})
...
test('id is required', () => {
  ...
  expect(() => new User()).toThrow(
    'Proxy should be instantiated with an id, but got: undefined.'
  )
})
...
describe('getters caching', () => {
  ...
  test('asynchronous', async() => {
    const foo = jest.fn(() => Promise.resolve('bar'))
...
describe('dataValues getter', () => {
  ...
  test('Entity does not exist', async() => {
    ...
    await expect(user.dataValues).rejects.toThrow(
      'Entity User with id "foo" does not exist.'
    )
  })
...
describe('exists getter', () => {
  test('exists', async() => {
    const User = createProxy({ getters: { dataValues() {
...
describe('clearCache method', () => {
  test('entityLoader has no clear function', async() => {
    const User = createProxy({ getters: { entityLoader() {
      return {}
    } }})
    const user = new User('foo')
  ...
  test('is clearing cache', () => {
    const clear = jest.fn()
    const foo = jest.fn(() => 'bar')
    const User = createProxy({ getters: { foo, entityLoader() {
      return { clear }
    } }})
